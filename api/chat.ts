...
const fallbackReplies = [
  ...
  "دوائرى تعمل بكفاءة. جرّب اختياراً جديداً، فقد تكون الإجابة مخبأة في التفاصيل.",
...
export default async function handler(req: any, res: any) {
  ...
  if (!parsed.success) return res.status(400).json({ error: "الرسالة غير صالحة" });
      ...
      const response = await fetch(`${apiBase.replace(/\/$/, "")}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: "أنت BMO، رفيق مغامرات عربي هادئ ومرح دون مبالغة. أجب بالعربية في جملتين أو ثلاث، اربط إجابتك بسياق اللعبة إن وجد، ولا تدّعي أنك شخصية رسمية من عمل محمي.",
            },
            ...history.slice(-6),
